...
export default class FCL_ListaSolicitudesDeMejora extends LightningElement {
  @track solicitudes = [];
  @track error;
  ...
  wiredSolicitudes({ error, data }) {
    if (data) {
      ...
      this.filteredSolicitudes = [...this.solicitudes]; // Inicializamos las solicitudes filtradas
  ...
  handleExport() {
    ...
    const exportData = this.filteredSolicitudes.map((item) => {
      const row = {};
      this.columns.forEach((col) => {
        let value =
          col.fieldName === "solicitudUrl" ? item.Name : item[col.fieldName];
    ...
    const wb = {
      Sheets: { "Solicitudes de Mejora": ws },
      SheetNames: ["Solicitudes de Mejora"]
    };
    const fecha = new Date().toISOString().slice(0, 10); // formato YYYY-MM-DD
    XLSX.writeFile(wb, `SolicitudesDeMejoraExportados_${fecha}.xlsx`);
  }
}