...
import obtenerDatosGrafico from '@salesforce/apex/fCL_CasesChartViewOnTime2.obtenerDatosGrafico';
...
import getTipoRequerimientoOptions from '@salesforce/apex/fCL_CasesChartViewOnTime2.getTipoRequerimientoOptions';
...
export default class FCL_ChartByGroupOnTime extends LightningElement {
    ...
    @wire(obtenerDatosGrafico, { anio: '$filters.anio', meses: '$filters.meses', tipoRequerimiento: '$filters.tipoRequerimiento', estados: '$filters.estado', unidad: '$filters.unidad', servicios: '$filters.servicio', gerencia: '$filters.gerencia' })
    wiredCases({ error, data }) {
        if (data) {
            console.log('Datos recibidos: ', JSON.stringify(data, null, 2)); // Log detallado
            ...
            // Limitar a 100 casos para la tabla
            this.displayedCases = this.cases.slice(0, 100);
            //console.log('Casos procesados:', this.cases);
            this.error = undefined; // Limpiar errores previos
        } else if (error) {
            console.error('Error al cargar los casos:', JSON.stringify(error, null, 2)); // Imprime el error completo
            this.error = error;
    ...
    wiredRequerimientos({ error, data }) {
        if (data) {
            this.tipoRequerimientoOptions = [{ label: '', value: null, disabled: true }, ...data.map(tr => ({ label: tr, value: tr}))];
        }else if (error) {
            console.error('Error al obtener las opciones de tipo de requerimiento: ', error);
        }
    }
    ...
    wiredEstados({ error, data }) {
        if (data) {
            this.estadoOptions = [{ label: '', value: null, disabled: true }, ...data.map(e => ({ label: e, value: e }))];
        } else if (error) {
    ...
    wiredUnidads({ error, data }) {
        if (data) {
            this.unidadOptions = [
                { label: "", value: null, disabled: true }, ...data.unidadesAsociadas.map((u) => ({ label: u, value: u }))];
        } else if (error) {
    ...
    wiredServicios({ error, data }) {
        if (data) {
            this.servicioOptions = [{ label: "", value: null, disabled: true }, ...data.subunidadesAsociadas.map((s) => ({ label: s, value: s }))];
        } else if (error) {
    ...
    wiredGerencias({ error, data }) {
        if (data) {
            this.gerenciaOptions = [{ label: "", value: null, disabled: true }, ...data.gerenciasAsociadas.map((g) => ({ label: g, value: g }))];
        } else if (error) {
    ...
    handleFilterChange(event){
        ...
        const filterValue = event.detail.value; // Valor del filtro
    ...
    loadChartData() {
        ...
        obtenerDatosGrafico({
            ...
            servicios: Array.isArray(this.filters.servicio) ? this.filters.servicio.join(',') : this.filters.servicio || null, // Validar si es array antes de usar join
        ...
        .then(result => {
            ...
            const labels = datos.map(item => item.tipoRequerimiento);
            const pendientes = datos.map(item => item.pendiente);
            const vencidos = datos.map(item => item.vencido);
            const resueltos = datos.map(item => item.resuelto);
                        ...
                        yAxes: [
                            {
                                ...
                                scaleLabel: {
                                    display: true,
                                    labelString: "Cantidad de Requerimientos"
                                }
                            }
                        ],
                          xAxes: [
                            {
                                ...
                                scaleLabel: {
                                    display: true,
                                    labelString: "Tipos de requerimientos"
                                }
                            }
                        ]
                    },
    ...
    sortData(fieldName, direction) {
        let sortedData = [...this.displayedCases];
