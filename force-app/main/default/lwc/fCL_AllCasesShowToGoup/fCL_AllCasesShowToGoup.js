...
import getAllCasesForUserGroups from '@salesforce/apex/FCL_CasesListController.getAllCasesForUserGroups';
...
export default class AllCasosList extends LightningElement {
    @track cases = [];
    @track error;
    @track filteredCases = [];
    ...
    @wire(getAllCasesForUserGroups, { userId: USER_ID })
    ...
    wiredEstados({error, data}){
        if(data){
            this.estadoOptions = [{label:'', value:'', disabled: true}, ...data.map(e=>({label:e, value:e}))];
        }else if(error){
    ...
    applyFilters(){
        this.filteredCases = this.cases.filter(item=>{
            return (
                ...
                (!this.selectedEstado || item.Status === this.selectedEstado) &&
